...
const StorePage: React.FC = () => {
  const { addToCart, books } = useAppContext();
  ...
  const filteredAndSortedBooks = useMemo(() => {
    ...
    if (selectedPriceFilter === 'Free') {
      filteredBooks = filteredBooks.filter(book => book.price === 0);
  ...
  }, [searchTerm, selectedGenre, selectedPriceFilter, sortBy, books]);

  const featuredBook = useMemo(() => {
    return (books || []).find(b => b.title.includes("Manual")) || (books || []).find(b => b.price > 0) || books?.[0];
  }, [books]);
