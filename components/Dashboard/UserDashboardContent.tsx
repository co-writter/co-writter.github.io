                        ...
                        {user.purchaseHistory && user.purchaseHistory.length > 0 ? (
                            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-x-4 md:gap-x-6 gap-y-8 md:gap-y-12">
                             ...
                             <div className="w-full py-32 border border-dashed border-white/10 flex flex-col items-center justify-center text-center bg-black/20 backdrop-blur-sm rounded-[32px]">
                                ...
                                <button onClick={() => navigate('/store')} className="px-8 py-3 bg-white text-black font-bold text-xs uppercase tracking-widest hover:bg-neutral-200 transition-colors rounded-full shadow-glow-white">
                                    Browse Books
                                </button>
                            </div>
                        )}
                         ...
                         {user.wishlist && user.wishlist.length > 0 ? (
                            user.wishlist.map((book: EBook) => (
                                <div key={book.id} className="flex items-center gap-5 p-5 border border-white/10 bg-black/40 backdrop-blur-md hover:border-white/30 transition-all group rounded-xl">
                                    ...
                                    <button onClick={() => navigate(`/store?bookId=${book.id}`)} className="px-6 py-2 border border-white/20 hover:bg-white hover:text-black text-white transition-colors text-[10px] font-bold uppercase tracking-widest rounded-full">
                                        View
                                    </button>
                                </div>
                            ))
                ...
                {currentTab === 'stats' && (
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-slide-up">
                          ...
                          <div className="bg-white/10 backdrop-blur-xl border border-white/10 text-white p-8 flex flex-col justify-between rounded-[32px]">
                              ...
                              <button onClick={() => navigate('/store')} className="mt-6 w-full py-3 bg-white text-black hover:bg-neutral-200 transition-colors text-xs font-bold uppercase tracking-widest rounded-full shadow-glow-white">
                                  Browse Books
                              </button>
                          </div>
                     </div>
