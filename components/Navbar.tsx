...
import { IconShoppingCart, IconRocket, IconMenu, IconX, IconStore, IconBook, IconHome, IconUser } from '../constants';
...
const Navbar: React.FC = () => {
    ...
    return (
        <>
            ...
            <nav
                className={`fixed top-0 left-0 right-0 w-full z-50 h-16 border-b transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] animate-slide-down ${scrolled
            ...
            >
                <div className="max-w-[1400px] mx-auto px-6 h-full flex items-center justify-between">
                    ...
                    {/* Logo */}
                    <Link to="/" className="flex items-center gap-3 group z-50 relative" aria-label="co-writter home">
                    ...
                    <div className="hidden md:flex items-center gap-8 absolute left-1/2 -translate-x-1/2">
                        {navLinks.map(link => (
                            <Link
                                key={link.path}
                                to={link.path}
                                className={`text-sm font-medium transition-all duration-300 flex items-center gap-2 hover:scale-105 ${isActive(link.path)
                                        ...
                                        <>
                                            ...
                                            <div className="absolute right-0 top-12 mt-2 w-56 bg-[#0a0a0a] border border-white/10 shadow-2xl py-1 z-20 origin-top-right rounded-lg animate-slide-up-stagger">
                                                ...
                                                <Link to="/dashboard" onClick={() => setDropdownOpen(false)} className="block px-4 py-2 text-sm text-neutral-300 hover:bg-white/5 hover:text-white transition-colors">
                                                    <div className="flex items-center gap-2"><IconUser className="w-4 h-4" /> My Profile</div>
                                                </Link>
                                                <Link to="/ebook-studio" onClick={() => setDropdownOpen(false)} className="block px-4 py-2 text-sm text-neutral-300 hover:bg-white/5 hover:text-white transition-colors lg:hidden">
                                                    <div className="flex items-center gap-2"><IconRocket className="w-4 h-4" /> Write Book</div>
                                                </Link>
